...
class PGClient:
    ...
    async def create_user(self, name: str, password: str) -> UserEntity:
        if await self.check_user_exist_by_name(name):
            raise UserExistsError(name)    
        user = UserEntity(name=name, password=password)
    ...
    """
        文档相关操作
    """
    async def create_document(self, document: DocumentEntity) -> DocumentEntity:
        self.session.add(document)
        await self.session.flush()
        await self.session.refresh(document)     
        return document

    async def get_document(self, doc_id: str) -> Optional[DocumentEntity]:
        return await self.session.get(DocumentEntity, doc_id)

    async def update_document(self, document: DocumentEntity) -> DocumentEntity:
        self.session.add(document)
        await self.session.flush()
        await self.session.refresh(document)
        return document
    ...
    """
        文档版本相关操作
    """
    async def create_document_version(self, version: DocumentVersionEntity) -> DocumentVersionEntity:
        self.session.add(version)
        await self.session.flush()
        await self.session.refresh(version)
        return version

    async def get_document_version(self, version_id: str) -> Optional[DocumentVersionEntity]:
        return await self.session.get(DocumentVersionEntity, version_id)    
    
    async def list_document_versions(self, doc_id: str) -> List[DocumentVersionEntity]:  
        statement = select(DocumentVersionEntity).where(DocumentVersionEntity.doc_id == doc_id).order_by(DocumentVersionEntity.create_time.desc())
    ...
    async def get_novel_tree_by_novel_id(self, novel_id: str) -> List[TreeSortEntity]:
