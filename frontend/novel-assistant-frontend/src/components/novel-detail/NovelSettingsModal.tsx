 ...
 */
export default function NovelSettingsModal({ isOpen, onClose, novel, onUpdate }: NovelSettingsModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // In a real app, upload first -> get URL -> update novel
      const coverUrl = formData.cover ? formData.coverUrl : formData.coverUrl;

      const updated = await updateNovel({
        user_id: userId,
        novel_id: novel.id,
        novel_name: formData.title,
        novel_summary: formData.synopsis,
        novel_cover_image_url: coverUrl
      });

      onUpdate(updated);
      onClose();
      logger.info('Updated novel:', novel.id);
    } catch (err: unknown) {
      logger.error('Failed to update novel:', err);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-surface-white w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden animate-scale-up flex flex-col max-h-[90vh]">
