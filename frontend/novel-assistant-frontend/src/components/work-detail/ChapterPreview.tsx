...
// TODO: 可能需要更加准确的组件化。
const ChapterPreview: React.FC<ChapterPreviewProps> = ({ chapter, onEdit, onSelectVersion }) => {
  ...
  const displayVersionName = currentVersion 
    ? (currentVersion.version || currentVersion.id) // Fallback for old data
    : 'v1.0';
                            ...
                            {chapter.versions.map(v => (
                                ...
                                >
                                    <div className="flex flex-col gap-0.5">
                                        <span className="font-bold">{v.version || v.id}</span>
