 ...
 * 更新时间: 2026-01-31 14:15
 * 更新记录:
 * - [2026-01-31 14:15:FE-REF-20260131-01: 在何处使用: 作品详情目录组件；如何使用: 替换Novel为Work terminology；实现概述: 重命名接口和组件，更新类型引用。]
...
interface WorkDirectoryProps {
  ...
  onCreateVolume: (name: string) => Promise<{ id: string; title: string } | void>;
  onCreateChapter: (volumeId?: string, name?: string) => Promise<{ id: string; title: string } | void>;
...
// 目录
const WorkDirectory: React.FC<WorkDirectoryProps> = ({
  volumes,
  orphanChapters,
  selectedChapterId,
  onSelectChapter,
  ...
  // Creation State
  const [creatingType, setCreatingType] = useState<'volume' | 'chapter' | null>(null);
  ...
  const handleCreateSubmit = async () => {
      ...
      if (creatingType === 'volume') {
          await onCreateVolume(createInputValue);
      } else if (creatingType === 'chapter') {
          await onCreateChapter(creatingParentId || undefined, createInputValue);
      }
        ...
        {editingId === chapter.id ? (
            ...
            <div className="flex items-center justify-between w-full overflow-hidden">
                ...
                <span className={`text-[10px] shrink-0 ml-2 px-1.5 py-0.5 rounded-full ${selectedChapterId === chapter.id ? 'bg-white/10 text-stone-300' : 'bg-stone-200 text-stone-500'}`}>
                    {chapter.currentVersionName || chapter.versions.find(v => v.id === chapter.currentVersionId)?.version || 'v1.0.0'}
                </span>
            </div>
        )}
    </div>
  ...
  const renderCreationInput = (isVolume: boolean) => (
      <div className={`flex items-center gap-2 py-2 px-2 ${isVolume ? '' : 'ml-4'}`}>
        <div className="p-0.5">
            {isVolume ? <ChevronRight className="w-4 h-4 text-stone-300" /> : <FileText className="w-4 h-4 text-stone-300" />}
        </div>
        <input 
            autoFocus
            className="bg-transparent border-b border-stone-300 outline-none w-full text-sm font-medium"
            placeholder={isVolume ? "输入卷名称" : "输入章节名称"}
            ...
            <div key={volume.id} className="mb-1">
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${volume.isExpanded ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="border-l border-stone-200 ml-4 pl-1 my-1 space-y-0.5">
                        ...
                        {creatingType === 'chapter' && creatingParentId === volume.id && renderCreationInput(false)}
                    </div>
                </div>
            </div>
        ))}
        {creatingType === 'volume' && renderCreationInput(true)}

        {/* Orphan Chapters (Always below volumes) */}
        {(orphanChapters.length > 0 || (creatingType === 'chapter' && !creatingParentId)) && (
             <div className="mt-2 pt-2 border-t border-stone-100">
                ...
                {creatingType === 'chapter' && !creatingParentId && renderCreationInput(false)}
             </div>
        )}
        
        {/* Empty State */}
        {volumes.length === 0 && orphanChapters.length === 0 && !creatingType && (
                    ...
                    <button 
                        ...
                        onClick={() => { 
                            if (contextMenu.targetId) {
                                onUpdateVolume(contextMenu.targetId, { isExpanded: true });
                            }
                            setCreatingType('chapter');
                            setCreatingParentId(contextMenu.targetId || null);
                            setCreateInputValue('');
                            setContextMenu(prev => ({ ...prev, visible: false })); 
                        }}
                    >
