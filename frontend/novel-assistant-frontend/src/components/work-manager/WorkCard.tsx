...
const WorkCard: React.FC<WorkCardProps> = ({ 
  ...
  const [editStatus, setEditStatus] = useState<Work['status']>(work.status);

  // Reset state when active status changes or work changes
  React.useEffect(() => {
    if (!isActive) {
      setMode('view');
    }
  ...
  return (
    ...
    `}>
      ...
      `}>
        ...
        {/* Yellow: Edit */}
        <button 
          onClick={(e) => { e.stopPropagation(); setMode('edit'); setEditTitle(work.title); setEditStatus(work.status); }}
        ...
        {/* Green: Plugin Config */}
        <button 
          onClick={(e) => { e.stopPropagation(); onOpenPluginConfig?.(work); }}
      ...
      {mode === 'delete' && (
        ...
        >
          <h4 className="text-sm font-medium text-gray-800 mb-4 text-center">
            是否确认删除<br/>
            <span className="font-bold text-black">“{work.title}”</span>
          </h4>
      ...
      {mode === 'edit' && (
        ...
        >
          ...
          <div className="mb-4">
             <label className="text-xs text-gray-500 mb-2 block">作品状态</label>
      ...
      <div className={`flex-1 p-5 flex flex-col justify-between bg-surface-white transition-all duration-300 ${mode !== 'view' ? 'blur-sm opacity-50' : ''}`}>
        <div>
          <h3 className="text-xl font-serif font-bold text-text-primary mb-3 line-clamp-1" title={work.title}>
            {work.title}
          </h3>
          
          <div className="flex items-center justify-between">
            <p className="text-sm text-text-secondary font-sans">
              作品类型: <span className="font-medium text-text-primary">{work.type || '通用'}</span>
            </p>
        ...
        <div className="border-t border-border-primary pt-3 mt-2">
          <p className="text-xs text-text-secondary">
            最新更新: {new Date(work.updatedAt).toLocaleDateString()}
          </p>
        </div>
      </div>
    </div>
