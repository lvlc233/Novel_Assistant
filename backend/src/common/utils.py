...
class DocumentTextSplitter:
    ...
    def _find_documents(self, text: str) -> List[Dict]:
        ...
        for i, line in enumerate(lines, 1):
            # 起始位置
            if re.match(self.document_pattern, line.strip()):
                documents.append({
                    'title': line.strip(),
                    'start_line': i,
        ...
        for i in range(len(documents)):
            if i < len(documents) - 1:
                documents[i]['end_line'] = documents[i + 1]['start_line'] - 1
            else:
                documents[i]['end_line'] = len(lines)
    ...
    def _extract_document_content(self, text: str, document_info: Dict) -> str:
        ...
        """
        lines = text.split('\n')
        start_idx = document_info['start_line']
        end_idx = document_info['end_line']
    ...
    def split_by_documents(self, file_path: str) -> List[Dict]:
        ...
        # 查找所有文档
        documents = self._find_documents(text)
        ...
        for document_info in documents:
            # 提取文档内容
            document_content = self._extract_document_content(text, document_info)
