...
interface WorkPluginConfigModalProps {
  work: Work;
  onClose: () => void;
 ...
 * 创建时间: 2026-03-05 15:35
 * 更新时间: 2026-03-05 15:35
 * 
 * 作品插件配置模态框 (WorkPluginConfigModal)
 * 
 * 在何处使用:
 * - 作品列表页或详情页，用于配置单个作品启用的插件及其参数。
 * 
 * 如何使用:
 * ```tsx
 ...
 * 内部实现梗概:
 * - 加载所有可用插件。
 * - 根据 `work.plugins` 初始化已选插件和配置。
 * - 提供左侧插件列表和右侧配置面板。
 * - 使用 `createPortal` 渲染到 `document.body`。
 */
export const WorkPluginConfigModal: React.FC<WorkPluginConfigModalProps> = ({ 
  work, 
  onClose, 
  onSave 
}) => {
  ...
  useEffect(() => {
    ...
    getPlugins().then(plugins => {
      ...
      // Override with work's saved plugins
      if (work.plugins) {
        work.plugins.forEach(p => {
          if (p.enabled) {
        ...
        <div className="h-16 px-6 border-b border-gray-100 flex items-center justify-between shrink-0 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">为《{work.title}》配置 AI 助手与工具</p>
            </div>
          </div>
          <button 
            onClick={onClose}
                  ...
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <h3 className="font-bold text-xl text-gray-900 mb-1">
                        {availablePlugins.find(p => p.id === activePluginId)?.manifest?.name || availablePlugins.find(p => p.id === activePluginId)?.name}
                      </h3>
                        ...
                        <div className="p-5 bg-blue-50 rounded-2xl flex gap-4 border border-blue-100">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-blue-600/80 leading-relaxed">
                              {availablePlugins.find(p => p.id === activePluginId)?.manifest?.description || availablePlugins.find(p => p.id === activePluginId)?.description}
                            </p>
                          </div>
                        </div>
...
export default WorkPluginConfigModal;
